...
Dedicated XGBoost-based anomaly detection system specifically optimized for CryoSat-2 satellite.
...
class CryoSat2AnomalyDetector:
    """
    XGBoost-based anomaly detection system specifically for CryoSat-2 satellite.
    
    This class is optimized for CryoSat-2's orbital characteristics and maneuver patterns.
    """
    
    def __init__(self, data_dir: str = '.'):
        """
        Initialize the CryoSat-2 anomaly detector.
        
        Args:
            data_dir: Directory containing orbital_elements and manoeuvres folders
        """
        self.data_dir = data_dir
        self.satellite_name = 'CryoSat-2'
        ...
        # CryoSat-2 optimized XGBoost parameters
        # These parameters are tuned for CryoSat-2's orbital characteristics
        self.xgb_params = {
        ...
        # CryoSat-2 specific feature engineering parameters
        self.lookback_days = 7  # Week lookback for ice mission patterns
        self.rolling_windows = [3, 7, 14, 21]  # Including 3-week window for seasonal patterns
    ...
    def load_cryosat2_data(self) -> bool:
        """
        ...
        # Load orbital data
        orbital_file = os.path.join(self.data_dir, 'orbital_elements', 'CryoSat-2.csv')
        if not os.path.exists(orbital_file):
            print(f"Error: CryoSat-2 orbital data not found at {orbital_file}")
            return False
        
        try:
            self.orbital_data = pd.read_csv(orbital_file, index_col=0, parse_dates=True)
            self.orbital_data.columns = self.orbital_data.columns.str.strip()
            self.orbital_data = self.orbital_data.sort_index()
            self.orbital_data = self.orbital_data[~self.orbital_data.index.duplicated(keep='first')]
            
            print(f"  ✓ Loaded orbital data: {len(self.orbital_data)} records")
            print(f"  ✓ Time range: {self.orbital_data.index.min()} to {self.orbital_data.index.max()}")
            print(f"  ✓ Available elements: {list(self.orbital_data.columns)}")
        ...
        # Load maneuver data
        maneuver_file = os.path.join(self.data_dir, 'manoeuvres', 'cs2man.txt')
            ...
            try:
                ...
                for line in lines:
                    ...
                    if len(parts) >= 8:
                        ...
                        self.maneuver_data.append(maneuver_time)
                
                self.maneuver_data = sorted(self.maneuver_data)
                print(f"  ✓ Loaded maneuver data: {len(self.maneuver_data)} maneuvers")
                
                if self.maneuver_data:
                    print(f"  ✓ Maneuver time range: {min(self.maneuver_data)} to {max(self.maneuver_data)}")
    ...
    def create_cryosat2_features(self, df: pd.DataFrame, target_col: str) -> pd.DataFrame:
        """
    ...
    def train_cryosat2_model(self, target_col: str, test_size: float = 0.2) -> Dict:
        """
        ...
        if self.orbital_data is None:
            raise ValueError("No orbital data loaded. Call load_cryosat2_data() first.")
        
        if target_col not in self.orbital_data.columns:
        ...
        feature_df = self.create_cryosat2_features(self.orbital_data, target_col)
    ...
    def detect_cryosat2_anomalies(self, target_col: str, threshold_method: str = 'std', 
    ...
    def evaluate_cryosat2_performance(self, target_col: str, anomaly_df: pd.DataFrame, 
        ...
        maneuver_times = pd.to_datetime(self.maneuver_data)
    ...
    def plot_cryosat2_results(self, target_col: str, anomaly_df: pd.DataFrame, 
    ...
    def run_full_cryosat2_analysis(self, target_col: str = 'eccentricity', 
        ...
        # Load data
        if not self.load_cryosat2_data():
            raise RuntimeError("Failed to load CryoSat-2 data")
        
        # Train model
        model_result = self.train_cryosat2_model(target_col)
        
        # Detect anomalies
        anomaly_df = self.detect_cryosat2_anomalies(target_col, threshold_method, threshold_factor)
        
        # Evaluate performance
        performance = self.evaluate_cryosat2_performance(target_col, anomaly_df)
        
        # Plot results
        if save_plots:
            plot_path = f"CryoSat-2_{target_col}_anomaly_detection.png"
            self.plot_cryosat2_results(target_col, anomaly_df, plot_path)
        else:
            self.plot_cryosat2_results(target_col, anomaly_df)
        ...
        full_results = {
            ...
            'data_summary': {
                'total_records': len(self.orbital_data),
                'time_range': f"{self.orbital_data.index.min()} to {self.orbital_data.index.max()}",
                'available_elements': list(self.orbital_data.columns),
                'total_maneuvers': len(self.maneuver_data)
            }
        }
        
        # Store results
        self.results[target_col] = full_results
        
        # Print comprehensive summary
        self.print_cryosat2_summary(full_results)
        
        return full_results
    
    def print_cryosat2_summary(self, results: Dict):
        """
...
def main():
    ...
    """
    print("CryoSat-2 Orbit Anomaly Detection System")
    ...
    for element, results in detector.results.items():
        metrics = results['model_metrics']
    ...
    print(f"\nCryoSat-2 anomaly detection analysis complete!")
